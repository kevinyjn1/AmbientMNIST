        ...
        if layer == 'CONV' and sparse:
            ...
            for m in mat:
                ...
                if data.shape[0] > 0:
                    ...
                    str_mat = ','.join(f_lit(d) for d in data.flatten())
                    mat_str += str_mat + ','
            ...
            mat_str = ','.join(f_lit(d) for d in data.flatten())
            mat_str = '{' + mat_str + '}'
            indices_str = '{' + ','.join(map(str, indices.flatten().tolist())) + '}'
            indptr_str = '{' + ','.join(map(str, indptr.flatten().tolist())) + '}'
            ...
            mat_str = '{' + ','.join(f_lit(d) for d in mat.flatten()) + '}'
...
def main(args):
    ...
    graph.append('conv1_wmd_', int_test.mul_np, 'conv1_wd', 'conv1_md')
    graph.append('conv1_wmd', int_test.permute, 'conv1_wmd_')
    graph.append('conv1_wmh_', int_test.mul_np, 'conv1_wh', 'conv1_mh')
    graph.append('conv1_wmh', int_test.permute_vh, 'conv1_wmh_')
    graph.append('conv1_wmv_', int_test.mul_np, 'conv1_wv', 'conv1_mv')
    graph.append('conv1_wmv', int_test.permute_vh, 'conv1_wmv_')

    graph.append('conv2_w', int_test.weight)
    graph.append('conv2_m', int_test.weight)
    graph.append('conv2_wm_', int_test.mul_np, 'conv2_w', 'conv2_m')
    graph.append('conv2_wm', int_test.permute, 'conv2_wm_')
    graph.append('conv2_b', int_test.weight)

    graph.append('fc1_wh', int_test.weight)
    graph.append('fc1_mh', int_test.weight)
    graph.append('fc1_wv', int_test.weight)
    graph.append('fc1_mv', int_test.weight)
    graph.append('fc1_b', int_test.weight)
    graph.append('fc1_wmh__', int_test.mul_np, 'fc1_wh', 'fc1_mh')
    graph.append('fc1_wmh_', int_test.transpose, 'fc1_wmh__')
    graph.append('fc1_wmh', int_test.shuffle, 'fc1_wmh_')
    graph.append('fc1_wmv_', int_test.mul_np, 'fc1_wv', 'fc1_mv')
