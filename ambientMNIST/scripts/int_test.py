...
def weight(name):
    #global file_path
    file_path = '../params/mnist/'
    path = os.path.join(file_path, name + '.param')
    with open(path, 'rb') as f:
        return to_fixed_vectorized(pickle.load(f, encoding='latin1'))
...
def rand_matrix(shape):
    return np.random.randint(-5, 5, size=shape)
...
def main(args):
    ...
    # Debugging statement to check the value of file_path
    print(f"Source directory: {file_path}")

    # Ensure file_path is not None
    if file_path is None:
        raise ValueError("The source directory (src_dir) must be provided and cannot be None.")
